...
Reward function for the interleaving and sequential tasks.
...
If numba is not available, falls back to pure Python implementations.
...
Performance:
- Uses numba JIT compilation when available (~100x speedup)
...
try:
    ...
    HAS_NUMBA = False
    # Define dummy decorator for fallback
...
GAP_EXTEND = -1.0    # Cost to extend a gap
...
class Vocabulary:
    """Maps words to integers for fast comparison."""
...
def _nw_score_only_affine_fallback(
    seq1: np.ndarray,
    seq2: np.ndarray,
    match: float,
    mismatch: float,
    ...
    """
    Pure Python fallback for _nw_score_only_affine when numba is not available.
    """
    n, m = len(seq1), len(seq2)
...
def _nw_full_affine_fallback(
    ...
    """
    Pure Python fallback for _nw_full_affine when numba is not available.
    """
    n, m = len(seq1), len(seq2)
...
def nw_align(
    ...
    if HAS_NUMBA:
        ...
        score, M, X, Y = _nw_full_affine_fallback(
            seq1_ids, seq2_ids,
            MATCH_SCORE, MISMATCH_PENALTY,
            GAP_OPEN, GAP_EXTEND
        )
...
def compute_alignment_score(
    ...
    if HAS_NUMBA:
        ...
        raw_score = _nw_score_only_affine_fallback(
            seq1_ids, seq2_ids,
            MATCH_SCORE, MISMATCH_PENALTY,
            GAP_OPEN, GAP_EXTEND
        )
    ...
    if args.benchmark:
        ...
        for _ in range(n_runs):
            if HAS_NUMBA:
                ...
                score = _nw_score_only_affine_fallback(seq1_ids, seq2_ids, MATCH_SCORE, MISMATCH_PENALTY, GAP_OPEN, GAP_EXTEND)
